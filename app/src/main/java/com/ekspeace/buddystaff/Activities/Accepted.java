...
public class Accepted extends AppCompatActivity implements IAcceptLoadListener {
    private Toolbar toolbar;
    private TextView headerText;
    private RecyclerView recyclerView;
    private LinearLayout loading;
    private IAcceptLoadListener iAcceptLoadListener;
    ...
    private void initView(){
        toolbar = findViewById(R.id.accept_toolbar);
        headerText = findViewById(R.id.accept_header_text);
        recyclerView = findViewById(R.id.accept_recycler_view);
        loading = findViewById(R.id.accept_progressbar);
        iAcceptLoadListener = this;
    ...
    private void loadAcceptedServices(){
        ...
        if (Common.isOnline(this)) {
            if(Common.currentInfoName == "Booking") {
                headerText.setText("Booking accept requests");
                            ...
                            public void onComplete(@NonNull @NotNull Task<QuerySnapshot> task) {
                                if (task.isSuccessful()) {
                                    if (!Objects.requireNonNull(task.getResult()).isEmpty()) {
                                        ...
                                        iAcceptLoadListener.onAcceptLoadSuccess(bookingInformationList, null);
                                    }
                                    loading.setVisibility(View.GONE);
                                }
                            }
                        }).addOnFailureListener(e -> iAcceptLoadListener.onAcceptLoadFail(e.getMessage()));
            }else {
                headerText.setText("Pick up accept requests");
                            ...
                            public void onComplete(@NonNull @NotNull Task<QuerySnapshot> task) {
                                if (task.isSuccessful()) {
                                    if (!Objects.requireNonNull(task.getResult()).isEmpty()) {
                                        ...
                                        iAcceptLoadListener.onAcceptLoadSuccess(null, pickInformationList);
                                    }
                                    loading.setVisibility(View.GONE);
                                }
                            }
        ...
        if (Common.isOnline(this)) {
            ...
            BookingInfo.delete().addOnFailureListener(new OnFailureListener() {
                ...
                public void onFailure(@NonNull Exception e) {
                    PopUp.Toast(Accepted.this, layout, R.drawable.error,e.getMessage(),Toast.LENGTH_SHORT);
                    loading.setVisibility(View.GONE);
                }
    ...
    private void deleteBookingFromUser(BookingInformation bookingInformation) {
        if (Common.isOnline(this)) {
            ...
            userBookingInfo1.delete().addOnFailureListener(e -> {
                ...
                loading.setVisibility(View.GONE);
                PopUp.Toast(Accepted.this, layout, R.drawable.success,"Successfully deleted the booking !",Toast.LENGTH_SHORT);
    ...
    private void deletePickUpFromUser(PickInformation pickInformation) {
        if (Common.isOnline(this)) {
            ...
            userPick.delete().addOnFailureListener(e -> {
                ...
                loading.setVisibility(View.GONE);
                PopUp.Toast(Accepted.this, layout, R.drawable.success,"Successfully deleted a pick up !",Toast.LENGTH_SHORT);
    ...
    @Override
    public void onAcceptLoadSuccess(List<BookingInformation> bookingInformations, List<PickInformation> pickInformations) {
        AcceptedAdapter adapter = new AcceptedAdapter(this, pickInformations, bookingInformations);
        recyclerView.setAdapter(adapter);
        loading.setVisibility(View.GONE);
    }

    @Override
    public void onAcceptLoadFail(String message) {
